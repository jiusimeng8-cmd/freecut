...
import { getCloudMcpConfig, isCloudMcpConfigured } from '@/shared/state/cloud-mcp-config-store'
...
/**
 * The media carries no speech, so there is nothing to caption.
 *
 * A distinct type because callers must treat it as a skip rather than a
 * failure: batch captioning should move on to the next clip instead of
 * reporting the whole run as broken.
 */
export class NoSpeechDetectedError extends Error {
  constructor(fileName?: string) {
    super(
      fileName
        ? `"${fileName}" 中没有检测到人声，已跳过字幕生成。`
        : '这段媒体中没有检测到人声，已跳过字幕生成。',
    )
...
class MediaTranscriptionService {
  ...
  async transcribeMedia(mediaId: string): Promise<MediaTranscript> {
    ...
    if (result.noSpeech || result.segments.length === 0) {
      throw new NoSpeechDetectedError(media.fileName)
    }
  ...
  async insertTranscriptAsCaptions(
    ...
    for (const clip of targetClips) {
      ...
      const previousVirtualStyleTemplate = previousVirtualStyle
        ? (definedCaptionStyleFields(previousVirtualStyle) as CaptionTextItemTemplate)
        : undefined
